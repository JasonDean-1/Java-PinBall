...
public class BallService
{
	...
	public void draw(Graphics g)
	{
		...
		} else
		{
			...
			// 绘制档板图像
			g.drawImage(stick.getImage(), stick.getX(), stick.getY(), stick.getPreWidth(),
					stick.getImage().getHeight(null), null);
			// 绘制弹球图像
				...
				for (int j = 0; j < bricks[i].length; j++)
				{
					...
					if (!bricks[i][j].isDisable())
					{
						...
						g.drawImage(bricks[i][j].getImage(), bricks[i][j].getX(), bricks[i][j].getY(),
								bricks[i][j].getImage().getWidth(null) - 1, bricks[i][j].getImage().getHeight(null) - 1,
								null);
